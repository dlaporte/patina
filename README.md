...
3. **Browse.** New sites show the patina's instant base theme, then upgrade to their custom LLM-generated theme within a few seconds. Everything after that is served from cache.
...
Content scripts run at `document_start` on every enabled site. On a cache hit, the stored theme applies before the page paints. On a first visit, the content script applies the patina's base theme, builds a small structural digest of the page (landmarks, common class names, a few computed colors and fonts), and hands it to the service worker, which asks your configured LLM for a theme envelope: a site-specific stylesheet plus widget placements. The envelope is validated against a closed widget catalog, the CSS is sanitized, and the result is cached per `(site, patina)` in `chrome.storage.local` — permanently, until you re-patinate or delete it.
